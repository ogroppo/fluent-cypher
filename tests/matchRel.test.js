...
test('Match rel and current rel', t => {
	...
	query.matchRel({alias: 'otherRel', minDepth: 1});
	query.matchRel({alias: 'otherRel', maxDepth: 2});
	query.matchRel({alias: 'otherRel', minDepth: 1, maxDepth: 2});
	t.is(query.queryString, 'MATCH ()-[otherRel*]->() MATCH ()-[rel*1]->() MATCH ()-[otherRel1*1..]->() MATCH ()-[otherRel2*..2]->() MATCH ()-[otherRel3*1..2]->() ');
});
