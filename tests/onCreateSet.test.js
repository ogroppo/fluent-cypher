...
test('#onCreateSet objects', t => {
	t.is(
    new CypherQuery().onCreateSet({$: 'node', log: 22, shol: 'bla', zu: ['12','21']}).queryString,
    'ON CREATE SET node.log = {log0}, node.shol = {shol1}, node.zu = {zu2} '
  )
  t.is(
    new CypherQuery().onCreateSet({$: 'node', log: {'<=': 22}}).queryString,
    'ON CREATE SET node.log <= {log0} '
  )
  t.deepEqual(
    new CypherQuery().onCreateSet({$: 'node', log: null, d: new Date('2019-12-12')}).queryParams,
