...
test('onMatchSet string', t => {
  ...
  'ON MATCH SET node.prop = ciccio, rel.prop = pasticcio ')
	t.is(
    new CypherQuery().onMatchSet({$: 'node', log: 22, shol: 'bla', zu: ['12','21']}).queryString,
    'ON MATCH SET node.log = {log0}, node.shol = {shol1}, node.zu = {zu2} '
  )
  t.is(
    new CypherQuery().onMatchSet({$: 'node', log: {'<=': 22}}, {$: '4', gg: 'lol'}).queryString,
    'ON MATCH SET node.log <= {log0}, 4.gg = {gg1} '
  )
  t.deepEqual(
    new CypherQuery().onMatchSet({$: 'node', log: null, d: new Date('2019-12-12')}).queryParams,
