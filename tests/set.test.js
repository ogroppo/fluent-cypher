...
test('#set objects', t => {
	t.is(
    new CypherQuery().set({alias: 'node', log: 22, shol: 'bla', zu: ['12','21']}).queryString,
    'SET node.log = {log0}, node.shol = {shol1}, node.zu = {zu2} '
  )
  t.is(
    new CypherQuery().set({alias: 'node', label: 'zio', labels: ['Po']}).queryString,
    'SET node:`Po`:`zio` '
  )
  t.deepEqual(
    new CypherQuery().set({alias: 'node', log: null, d: new Date('2019-12-12')}).queryParams,
